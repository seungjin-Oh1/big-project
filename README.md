...
- **전화 상담에 쓸 번호가 없다.** 연동 코드는 다 있고 회선 계약만 없다.
- **검색 실패와 결과 없음이 화면에서 구분되지 않는다.** RAG·분석의 예외를 삼키고
  빈 값을 돌려주기 때문이다. "못 찾았다"와 "없다"는 다른 말인데 같게 보인다. 같은
  성격의 문제가 누락자료·개인정보 가림·상담자 이름 표시에도 남아 있다.
- **누락자료 후보가 확신도 임계값에 걸려 흔들린다.** 같은 상담을 두 번 돌리면 건수가
  달라진다. 전부 걸러지는 경우는 막아 두었지만 개수의 편차는 남아 있다.
- **서버를 여러 대로 늘릴 수 없다.** 통화 세션·캡차·로그인 잠금이 프로세스 메모리에
  있고 Chroma도 로컬 파일이다. 늘리려면 그 상태들을 먼저 밖으로 빼야 한다.
- **테스트가 얇다.** 특히 프론트엔드와 core-api. 위 "테스트" 절 참고.
